      ...
      },
      {
        ...
        cellEditorParams: {
          getValidationErrors: (params) => {
            const tax = Number(params.value);
            const discount = Number(params.data?.discountPercent);
            if (isNaN(tax)) return ["Tax must be a number"];
            if (tax < 0 || tax > 100) return ["Tax must be between 0 and 100"];
            if (!isNaN(discount) && tax > discount)
              return ["Tax cannot exceed discount"];
            return null;
          },
        },
        tooltipValueGetter: (p) => p.validationErrors?.join(", "),
      },
      {
